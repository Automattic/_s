<?php 
/*
Template Name: As Stafff Contects
*/
get_header(); 
...
(SELECT organization_24 FROM civicrm_value_contact_fields_9 WHERE entity_id = ca.contact_id) as Organization,
(SELECT board_position_22 FROM civicrm_value_contact_fields_9 WHERE entity_id = ca.contact_id) as Position,
(SELECT hhs_region_29 FROM civicrm_value_contact_fields_9 WHERE entity_id = ca.contact_id) as Region,
(SELECT phone FROM civicrm_phone WHERE contact_id = ca.contact_id) as Phone,
...
WHERE cg.id = 2 and csp.abbreviation = '$dstate'"); */
// echo "<pre>";
// print_r($result);
// echo "</pre>";
...
/* print_r($_REQUEST);
$categoryUniversityID = $_POST['stateName'];
echo $categoryUniversityID; */
	...
	<div class="container">
		<div class="location-search">
			<div class="col-md-8">
				<div class="col-sm-5">
					<h4>State Search:</h4>
					<form action="" method="POST" id="state_contact_form" name="select_state_contact_form">
						<select name="ctl00$ContentPlaceHolder1$searchState" id="ContentPlaceHolder1_searchState">
							<option value="">Select State or Territory</option>
								<?php if ( $queryString->have_posts() ) {
									...
									while ( $queryString->have_posts() ) { 
										...
										?>
										<option value="<?php echo $post_link; ?>" id="state_contact_form_id"><?php echo $post_title; ?></option>
									<?php }
									wp_reset_query();
								}
							?>
						</select>
						<input type="submit" name="state_contact_submit" value="Submit"/>					
					</form>
				</div>
				<div class="col-sm-5 pull-right">
					<h4>Freeform Search:</h4>
					<form method="POST" action="" name="state_contact_key_word_form" id="state_contact_key_word_form_id">
						<input name="state_contact_key_word" type="text" id="ContentPlaceHolder1_tbxKeywords" placeholder="Keywords" <?php if (!empty($_POST['state_contact_key_word'])) { ?> value="<?php echo $_POST['state_contact_key_word']; ?>" <?php }; ?>/>
						<select name="state_contact_select_program" id="ContentPlaceHolder1_ddlProgram">
							<option <?php if ($_POST['state_contact_select_program'] == 'CSBG and WAP') { ?> selected="true" <?php }; ?> value="CSBG and WAP" >CSBG and WAP</option>
							<option <?php if ($_POST['state_contact_select_program'] == 'CSBG') { ?> selected="true" <?php }; ?>value="CSBG">CSBG</option>
							<option <?php if ($_POST['state_contact_select_program'] == 'WAP') { ?> selected="true" <?php }; ?>value="WAP">WAP</option>	
						</select>
						<input type="submit" name="state_contact_submit" id="state_contact_submit_id" value="Submit"/>
					</form>
				</div>
...
<div class="region-list-result" id="search_result">
	<div class="container">
	<!-- Region Listing -->
...
if(isset($_POST['state_contact_submit'])){	?>
		<script> jQuery('html, body').animate({scrollTop: jQuery("#search_result").offset().top}, 100);</script>
		<?php
	echo '<div id="results">';
		$searchKeyword = $_POST['state_contact_key_word'];
		$selectedProgram = $_POST['state_contact_select_program'];	
		...
		foreach($result_key_word as $response){
			...
			// echo "<pre>";
			// print_r($key_wap_array);
			// echo "</pre>";
			// echo "<pre>";
			// print_r($key_csbg_array);
			// echo "</pre>";
		...
		foreach($csbg_array as $data){
			...
			echo '<div class="region-result">'; ?>
				...
				<?php }?>
				 ...
				 <?php } ?>
				 <?php /*if(!empty( $region )) { ?>
				 <div class="link-region "><b>Region  :</b> <?php echo $region; ?></div>
				 <?php }*/	?>
			<?php echo '</div>';
