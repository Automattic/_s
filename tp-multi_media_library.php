<?php 
/*
 * Template Name: MultiMedia Library Page
 */
get_header(); 
					...
					<div class="media-list">
						<div class="row">
							...
							foreach( $rating as $ratings  ){
									...
									$video_type = get_field('video_type',$videoID);
									
									if($video_type == 'Thumb Url'){
										// Video Image
										...
										$display_video = apply_filters( 'the_content',get_the_content( $videoID ) );
									} else if($video_type == 'Upload File') {
										$upload_video = get_field('upload_video',$videoID);
										$video_image = get_field('upload_video_image',$videoID);
										
										if(!empty($video_image)){
											$video_image = $video_image['sizes']['success-story-post-image'];
											$video = '<video class="video" controls="" autoplay loop><source src="'.$upload_video['url'].'" type="video/mp4"></video>';
										}else if(!empty($featured_image)){
											$video_image = $featured_image;
										}																		
									}
									...
									?>
								<?php if(($video_type == 'Upload File' && !empty($upload_video['url'])) || !empty(($video_type == 'Thumb Url') && !empty($thumb_url))) { ?>
								<div class="col-sm-4">
									<div class="media-block">
										<div class="media-img">
										  <?php if($video_type == 'Thumb Url') { ?>
											   <a data-postid="<?php echo $videoID; ?>" data-src="#detailsModal-<?php echo $index; ?>" href="javascript:void(0);" class="postDetails"> <img src="<?php echo $thumb_url; ?>" alt="<?php echo $videoTitle; ?>"></a>
										   <?php } else if($video_type == 'Upload File' && !empty($upload_video['url'])) { ?>
											   <a data-postid="<?php echo $videoID; ?>" data-src="#detailsModal-<?php echo $index; ?>" href="<?php echo $upload_video['url']; ?>" class="fancy_video" data-poster="<?php echo $video_image; ?>"><img src="<?php echo $video_image; ?>" alt="<?php echo $videoTitle; ?>"></a>
										   <?php }?>
										</div>
						...
						<?php $index = 1;
							while ( $videoQry->have_posts() ) : $videoQry->the_post(); 
								...
								$videoLink = get_permalink($videoID); 
									...
									$featured_image = wp_get_attachment_image_url(get_post_thumbnail_id(),'video-thumb');	
									$video_type = get_field('video_type',$videoID);
									...
									if($video_type == 'Thumb Url'){
										...
										if(!empty($video_image)){
											$video_image = $video_image['sizes']['video-thumb'];											
										}else if(!empty($featured_image)){
											$video_image = $featured_image;
										}																			
									}
